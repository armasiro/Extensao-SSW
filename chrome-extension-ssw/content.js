...
function op101_justificativa(){
    ...
    novocampo11.setAttribute('style', 'font-family:verdana;font-size:10px;position:absolute;left:560px;top:344px;visibility:hidden')
    ...
    novocampo22.setAttribute('style', 'font-family:verdana;font-size:10px;position:absolute;left:560px;top:362px;visibility:hidden;')
    ...
    novocampo33.setAttribute('style', 'font-family:verdana;font-size:10px;color:black;position:absolute;left:560px;top:380px;visivility:hidden;');
    ...
    novocampo1.setAttribute('style', 'top:344px;font-family:verdana;font-size:10px;left:680px;position:absolute;visibility:hidden;');
    ...
    novocampo2 = document.createElement('input');
    novocampo2.setAttribute('type', 'text');
    novocampo2.setAttribute('id', 'novocampo2');
    novocampo2.setAttribute('style', 'top:362px;left:680px;width:44ch;color:grey;position:absolute;visibility:hidden;font-family:verdana;font-size:10px');
    novocampo2.setAttribute('placeholder', '(responsabilidade)');
    document.body.append(novocampo2);
    document.getElementById('novocampo2').disabled = true;
    ...
    novocampo3.setAttribute('style', 'top:380px;visibility:hidden;color:black;left:680px;width:44ch;font-family:verdana;position:absolute;font-size:10px;');
    document.body.append(novocampo3);

    justificativa = document.createElement('div');
    justificativa.setAttribute('style', 'font-family:verdana;font-size:10px;top:368px;left:700px;position:absolute;color:blue;');
    ...
    document.getElementById('novocampo1').addEventListener('change', ()=>{
        document.getElementById('novocampo2').value = document.getElementById('novocampo1').value;
    })
...
function cadastro_tabela() {
    grupo = document.getElementsByClassName("texto3")[0].innerText.substring(document.getElementsByClassName("texto3")[0].innerText.length - 2);
    if (grupo == '9' || grupo == '09' || grupo == '38' || grupo == '52') {
        ...
        grupo = document.getElementsByClassName("texto3")[0].innerText.substring(document.getElementsByClassName("texto3")[0].innerText.length - 2);
        usuario = getCookie('login');
        if (grupo == '09' || grupo == '9' || grupo == '52' || usuario == 'bruno') {
...
function op102_analises() {
    ...
    tabela2 = document.createElement('table');
    tabela2.setAttribute('id', 'tabela2')
    tabela2.setAttribute('style', 'margin-top:200px;margin-left:20px;')
    document.querySelector('body').append(tabela2)

    if (grupo == "43" || usuario == 'lidiane' || usuario == 'ligia' || usuario == 'erenilda' || usuario == 'luciana') {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                for (var i = 0; i < x.length; i++) {
                    ...
                    html += '<td class="srtd2"><div class="srdvl" id="d' + i + '">' + x[i].cte + '</div></td>';
                    html += '<td class="srtd2">' + x[i].rem + '</td>';
                    html += '<td class="srtd2">Aguardando análise</td>';
                    html += '<td class="srtd2">' + x[i].usuario + '</td>';
                    html += '<td class="srtd2">' + x[i].desde + '</td>';
                    html += '</tr>';
                }
            }
            document.getElementById('tabela2').innerHTML += html;
        ...
        xmlhttp.open("GET", "https://sswresponse.azurewebsites.net/?op=credito_exibe", true);
        xmlhttp.send();
        ...
        table2.addEventListener('click', function(e) {
            if (e.target.nodeName.toUpperCase() == "DIV") {
                if (e.target.id) {
                    document.getElementById("cnpj_cliente").value = e.target.innerHTML
        ...
        if (this.readyState == 4 && this.status == 200){
            ...
            if(x.length>0){
                if(x[0].status=='solicitando'){
                    ...
                    motivos = ['1-CADASTRO DE CLIENTE INCORRETO Alterou de alguma forma o frete (ICMS, cobrança TDE, taxas)', '2-COBRANÇA TAXA INDEVIDA Cobrado taxas indevidas ( tas, TDE, ICMS, TAD)', '3-MERCADORIA DE USO UND PROPRIA/PARCEIRA  Não linhou a cortesia e cobrou frete / Não emitiram cortesia', '4-REVERSÃO DE FRETE / ALTERA TABELA Emitiu NF tomador incorreto, reverteu, porém, solicita desconto de acordo com tabela', '5-ERRO DO CLIENTE  EMISSAO NF   Cliente emitiu NF errada na qual interferiu no frete ( Valor, peso / inscrição / cidade )', '6-ERRO DO CLIENTE AO ENVIAR ARQUIVO EDI    Enviou arquivo errado para a carvalima', '7-REAJUSTE INDEVIDO   Matriz reajustou indevido / Cliente de outro grupo / Não passado para cliente especial', '8-COTACAO NÃO ACATADA POR DIVERGENCIA / NÃO CONTRATADA    Alterou os dados, cliente de recusa pagar / Alterou dados, não foi alterado / Cotação não foi contratada /', '9-DUPLICIDADE EMISSAO COM A MESMA NF  Há mais de uma emissão com a mesma NF', '10-TAXA DE COLETA/ENTREGA INDEVIDA Cobrado TDE / TODA / Coleta / Outros indevido', '11-FALTA / ERRO DE AGRUPAMENTO NF  Não agrupou as Nfs no CTRC ( mesmo CNPJ remet/desti , coleta do mesmo dia, mesma cidade destino)', '12-EMISSAO REMET/ DEST/TOMADOR INCORRETO/ DADOS DIVERGENTE  Tomador incorreto na emissão / Dados divergente ( peso, NF, cidade), não seguiu a NF', '13-NAO RESPEITOU MENSAGEM DE TELA   Havia alguma particulidade com o cliente e não foi acatado', '14-PESO / CUBAGEM INCORRETO EMISSAO   Emitido com peso errado / Cubagem lançada manual errada (enviar fotos comprovando)', '15-COTACAO NAO ACATADA NA EMISSAO Cotação contratada, porém, não acatada', '16-EMISSAO FORA TABELA /TRANS/DEVOL   Emitido no Fob Dirigido / Promocional / Generica e não foi acatada a tabela do cliente tomador', '17-ATRASO NO CADASTRO DA TABELA   Matriz não cadastrou no periodo de 48 horas', '18-TABELA CADASTRADA ERRADA  Erro no cadastro da tabela no sistema', '19-FALHA DO SISTEMA SSW  TI    SSW calculou errado / Puxou alguma taxa indevida / Erro no arquivo enviado pelo cliente', '20-FALHA FORMALIZAÇÃO DA TABELA  Supervisão / Matriz confecionou a tabela errada', '21-ATRASO/ PROBLEMA COLETA/ ENTREGA  Cliente solicita desconto devido a atraso na coleta / entrega', '22-ERRO CUBAGEM NA AFERIÇÃO    Operação lançou a cubagem indevida no sistema ( na conferencia do peso, foi lançado as medidas errada)', '23-SEM CARIMBO EXPRESS    Motorista não carimbou EXPRESS na declaração (enviar fotos comprovando)', '24-PROMOCIONAL COM RECUSA    Cliente se recusa em pagar, alegando valor alto (seguir o manual promocional)', '25-OUTROS  DETALHE Caso não se enquandre em uns dos motivos acima, detalhe o motivo do seu desconto'];
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (x.length > 0) {
                if (x[0].status == 'solicitado') { //exibe apenas para 9, 38 e 52
                    ...
                    if (grupo == '9' || grupo == '38' || grupo == '52' || grupo == '09') {
                        campo = document.createElement('span')
                        ...
                        */
                        ...
                        motivos_campo.setAttribute('id', 'motivo_desconto');
                        motivos = ['1-CADASTRO DE CLIENTE INCORRETO Alterou de alguma forma o frete (ICMS, cobrança TDE, taxas)', '2-COBRANÇA TAXA INDEVIDA Cobrado taxas indevidas ( tas, TDE, ICMS, TAD)', '3-MERCADORIA DE USO UND PROPRIA/PARCEIRA  Não linhou a cortesia e cobrou frete / Não emitiram cortesia', '4-REVERSÃO DE FRETE / ALTERA TABELA Emitiu NF tomador incorreto, reverteu, porém, solicita desconto de acordo com tabela', '5-ERRO DO CLIENTE  EMISSAO NF   Cliente emitiu NF errada na qual interferiu no frete ( Valor, peso / inscrição / cidade )', '6-ERRO DO CLIENTE AO ENVIAR ARQUIVO EDI    Enviou arquivo errado para a carvalima', '7-REAJUSTE INDEVIDO   Matriz reajustou indevido / Cliente de outro grupo / Não passado para cliente especial', '8-COTACAO NÃO ACATADA POR DIVERGENCIA / NÃO CONTRATADA    Alterou os dados, cliente de recusa pagar / Alterou dados, não foi alterado / Cotação não foi contratada /', '9-DUPLICIDADE EMISSAO COM A MESMA NF  Há mais de uma emissão com a mesma NF', '10-TAXA DE COLETA/ENTREGA INDEVIDA Cobrado TDE / TODA / Coleta / Outros indevido', '11-FALTA / ERRO DE AGRUPAMENTO NF  Não agrupou as Nfs no CTRC ( mesmo CNPJ remet/desti , coleta do mesmo dia, mesma cidade destino)', '12-EMISSAO REMET/ DEST/TOMADOR INCORRETO/ DADOS DIVERGENTE  Tomador incorreto na emissão / Dados divergente ( peso, NF, cidade), não seguiu a NF', '13-NAO RESPEITOU MENSAGEM DE TELA   Havia alguma particulidade com o cliente e não foi acatado', '14-PESO / CUBAGEM INCORRETO EMISSAO   Emitido com peso errado / Cubagem lançada manual errada (enviar fotos comprovando)', '15-COTACAO NAO ACATADA NA EMISSAO Cotação contratada, porém, não acatada', '16-EMISSAO FORA TABELA /TRANS/DEVOL   Emitido no Fob Dirigido / Promocional / Generica e não foi acatada a tabela do cliente tomador', '17-ATRASO NO CADASTRO DA TABELA   Matriz não cadastrou no periodo de 48 horas', '18-TABELA CADASTRADA ERRADA  Erro no cadastro da tabela no sistema', '19-FALHA DO SISTEMA SSW  TI    SSW calculou errado / Puxou alguma taxa indevida / Erro no arquivo enviado pelo cliente', '20-FALHA FORMALIZAÇÃO DA TABELA  Supervisão / Matriz confecionou a tabela errada', '21-ATRASO/ PROBLEMA COLETA/ ENTREGA  Cliente solicita desconto devido a atraso na coleta / entrega', '22-ERRO CUBAGEM NA AFERIÇÃO    Operação lançou a cubagem indevida no sistema ( na conferencia do peso, foi lançado as medidas errada)', '23-SEM CARIMBO EXPRESS    Motorista não carimbou EXPRESS na declaração (enviar fotos comprovando)', '24-PROMOCIONAL COM RECUSA    Cliente se recusa em pagar, alegando valor alto (seguir o manual promocional)'];
                        motiv = document.createElement('option');
                        motiv.text = '';
                        motivos_campo.appendChild(motiv);
                        for (i = 0; i < motivos.length; i++) {
                    ...
                    solicitacao.innerHTML = "<b>Desconto solicitado - Por: " + x[0].usuario_solicitante + " -<br>Valor: R$" + x[0].valor_desconto + "</b>";
                ...
                solicitarA.innerHTML = "Solicitar Desconto";
                ...
                document.getElementById('solicitar_descontoA').addEventListener('click', () => {
                    ...
                    document.getElementById('solicita_titulo').style.visibility = 'visible';
                ...
                campo.setAttribute('id', 'solicita_titulo');
                document.querySelector('body').append(campo);
                campo = document.createElement('span');
                ...
                detalhec.setAttribute('style', 'position:absolute:left1140px;top:655px;color:black;font-size:12px;font-family:verdana;width:20ch;');
                document.querySelector('body').append(detalhec);
                detalhet = document.createElement('span');
                detalhet.setAttribute('id', 'detalhet');
                detalhet.setAttribute('style', 'position:absolute:left:1020px;top:655px;color:black;font-size:12px;font-family:verdana;');
                ...
                document.getElementById('botaosolicitar').addEventListener('click', () => {
                    ...
                    if (valor_desconto == '' || data_solicitacao == '' || motivo_desconto == '' || nome_clienteD == '' || telefone_cliente == '' || email_cliente == '' || ctrcfatura == '') {
                        alert('Preencha todos os campos.')
                    } else {
                        obj = {
                            valor_desconto: valor_desconto,
                        ...
                        console.log(obj)
                        salva_bd(obj, 'desconto');
                        alert('Desconto solicitado com sucesso.')
                ...
                document.getElementById('solicita_titulo').style.visibility = 'hidden';
...
function op102_analise() { //ANÁLISES DE CRÉDITO
    setInterval(function() {
        if (getCookie("clicar") == "ocorrencia") {
    ...
    if (telap == "ssw0840.02") { // Se for a tela das ocorrências
        grupo = document.getElementsByClassName("texto3")[0].innerText.substring(document.getElementsByClassName("texto3")[0].innerText.length - 2);
        if (grupo == '43' || grupo == '52') { //e se o grupo for o financeiro (43)
            ...
            xmlhttp = new XMLHttpRequest(); //BUSCA NO BD E EXIBE O STATUS DA ANÁLISE                           
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    myObj = this.responseText;
                    x = JSON.parse(myObj);
                    if (x.length > 0) { //SE O REGISTRO EXISTIR NO BD: EXIBE:
                        if (x[0].status == 'Aguardando análise') {
                            //STATUS: AGUARDANDO ANÁLISE...
                            analise_status = document.createElement('div');
                            analise_status.setAttribute('style', 'left:830px;top:110px;text-align:left;font-family:verdana;')
                            analise_status.innerHTML = '<i>(Aguardando análise -  desde ' + x[0].desde + ')</i>';
                            document.getElementById('frm').append(analise_status);
                            analise_botao = document.createElement('a');
                            analise_botao.setAttribute('style', 'left:890px;top:140px;font-family:verdana;font-size:12px;color:blue;text-decoration:none');
                            analise_botao.setAttribute('href', '#');
                            analise_botao.setAttribute('id', 'analise_botao');
                            analise_botao.innerText = '►'
                            document.getElementById('frm').append(analise_botao);
                            document.getElementById('analise_botao').addEventListener('click', () => {
                                cnpj = document.getElementsByClassName('data')[0].innerText;
                                estado = "Em análise";
                                var data = new Date();
                            ...
                            document.getElementById('aprova_reprova').addEventListener('click', () => { //INICIO DO BOTÃO DE APROVAR OU REPROVAR
                                ...
                                if (document.getElementById('radioReprovado').checked == true) { //SE "REPROVADO" ESTIVER MARCADO, PERGUNTA O MOTIVO
                                    ...
                                    neoaprova_reprova.setAttribute('style', 'left:890px;top:175px;font-family:verdana;font-size:12px;color:red;text-decoration:none');
                                    ...
                                    document.getElementById('neoaprova_reprova').addEventListener('click', () => {
                        ...
                        } else { //SE A ANÁLISE ESTIVER RESOLVIDA, EXIBE O STATUS FINAL DELA
                            analise_status = document.createElement('div');
                            analise_status.setAttribute('style', 'text-align:left;left:830px;top:110px;font-family:verdana;')
                            analise_status.innerHTML = x[0].status + ' em ' + x[0].desde + ' - Por: ' + x[0].usuario;
                            document.getElementById('frm').append(analise_status);
                            analise_motivo = document.createElement('div');
                            analise_motivo.setAttribute('style', 'text-align:left;left:830px;top:135px;font-family:verdana;')
                            analise_motivo.setAttribute('id', 'analise_motivo');
                            analise_motivo.innerText = x[0].dest;
                            document.getElementById('frm').append(analise_motivo);
                        }
                    }
                }
            }
            xmlhttp.open("GET", "https://sswresponse.azurewebsites.net/?op=credito_analise&cnpj=" + cnpj, true);
            xmlhttp.send();
        }
...
function op457() { //NOVOS CAMPOS NA OPÇÃO 457
    ...
    alerta.setAttribute('style', 'position:absolute;left:10px;top:216px;font-family:verdana;color:red');
    alerta.innerHTML = '*Campos obrigatórios.'
    document.querySelector('body').append(alerta);

    document.getElementsByClassName('texto')[1].innerText = '*Texto';
    document.getElementsByClassName('texto')[1].style.top = '188px';
    document.getElementsByClassName('texto')[1].style.left = '10px';


    setInterval(() => {
        if (document.getElementById('1').value == '' || document.getElementById('stat').value == '') {
            document.getElementById('2').style.visibility = 'hidden';
        } else {
    ...
    novo = crEl('div', 'stats', '*Status da Ação:', 'texto', 'left:12px;top:152px') //Status da Ação
    document.getElementById('frm').append(novo)
    ...
    document.getElementById('2').addEventListener('click', (e) => {
        ...
        var data2 = '';
        var data3 = '';
        var texto = document.getElementById('1').value; //texto
...
function op002() { //CONFIGURA OPÇÃO CANCELAMENTO NA OP002
    ...
    motivos.style.fontSize = '10px';
    motivos.style.fontFamily = 'Verdana';
    motivos.style.width = '24ch';
    motivos.style.color = 'blue';
    motivos.style.left = '990px';
    motivos.style.top = '560px';
    ...
    */
    ...
    origem.setAttribute('style', 'left:582px;top:64px;font-size:10px;font-family:verdana');
    document.getElementById('frm').append(origem);

    origem_campo = document.createElement('select');
    origem_campo.setAttribute('style', 'left:632px;top:63px;font-size:10px;font-family:verdana;color:blue;border:1px solid blue;');
    origem_campo.setAttribute('id', 'origem_cliente');
    origens = ['Ligação', 'Whatsapp', 'E-mail', 'Presencial'];
...
function form_desconto() {
    ...
    motivos = ["1-CADASTRO DE CLIENTE INCORRETO", "2-COBRANÇA TAXA INDEVIDA", "3-MERCADORIA DE USO UND PROPRIA/PARCEIRA", "4-REVERSÃO DE FRETE / ALTERA TABELA", "5-ERRO DO CLIENTE  EMISSAO NF", "6-ERRO DO CLIENTE AO ENVIAR ARQUIVO EDI", "7-REAJUSTE INDEVIDO", "8-COTACAO NÃO ACATADA POR DIVERGENCIA / NÃO CONTRATADA", "9-DUPLICIDADE EMISSAO COM A MESMA NF", "10-TAXA DE COLETA/ENTREGA INDEVIDA", "11-FALTA / ERRO DE AGRUPAMENTO NF", "12-EMISSAO REMET/ DEST/TOMADOR INCORRETO/ DADOS DIVERGENTE", "13-NAO RESPEITOU MENSAGEM DE TELA", "14-PESO / CUBAGEM INCORRETO EMISSAO", "15-COTACAO NAO ACATADA NA EMISSAO", "16-EMISSAO FORA TABELA /TRANS/DEVOL", "17-ATRASO NO CADASTRO DA TABELA", "18-TABELA CADASTRADA ERRADA", "19-FALHA DO SISTEMA SSW  TI", "20-FALHA FORMALIZAÇÃO DA TABELA", "21-ATRASO/ PROBLEMA COLETA/ ENTREGA", "22-ERRO CUBAGEM NA AFERIÇÃO", "23-SEM CARIMBO EXPRESS", "24-PROMOCIONAL COM RECUSA", "25-OUTROS  DETALHE"];
    node = document.createElement("select");
    ...
    chrome.storage.local.get(['permissao'], function(result) {
        ...
        if (permissao.includes('op001_1') && dominio == 'OTC') {
            ...
            */
            ...
            texto = document.createTextNode("Checa novas coletas");
            node.appendChild(texto);
...
function declaracao() {
    ...
    if (getCookie('permissao').includes('declaracao') && dominio == 'OTC') {
        ...
        document.getElementById('59').addEventListener('blur', (e) => {
            if (document.getElementById('50').value.includes('ESPIG') || document.getElementById('50').value.includes('ESPE') || document.getElementById('50').value.includes('NOVA UNIAO')) {
